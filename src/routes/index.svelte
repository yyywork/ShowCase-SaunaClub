<script lang="ts">
  ...
  onMount(() => {
    getTop = function(p: string,y: number):number{
    ...
    getColor = function(pTop: number,which: string){
      let result: number = 0;
      if(pStop[which])
        return 1;
      if(pTop>(innerHeight/2)){
        result = Math.min(pTop/innerHeight,0.5);
      }else{
        result = Math.min((1 - pTop/innerHeight),1);
      }
      return result;
    }
    getPos = function(pTop: number,which: string){
      let result = 0;
      
      if(pStop[which])
        return 0+"%";
      if(pTop>(innerHeight/2)){
        result = Math.min(pTop/innerHeight * 100,pTop/innerHeight * 100) * -1;
      }else{
        // if scrolled to the bottom
        if((innerHeight+y)>=docH){
          result = 0
        }else{
          result = Math.max(pTop/innerHeight * 100 - 20,0) * -1;
        }
      }
  ...
  let y: number,y2: number,innerHeight: number, docH: number;
  
  let getTop = (p: string,y: number)=>{}
  let getColor = (pTop: number,which: string)=>{}
  let getPos = (pTop: number,which: string)=>{}
  ...
  <style type="text/scss" global>
    ...
    .twoParts:nth-child(even){
      ...
      // right: var(--even-pos);
      transform: translate(calc(var(--even-pos) * -1),0);
    }
    .twoParts:nth-child(odd){
      opacity: var(--odd-color);
      // left: var(--odd-pos);
      transform: translate(var(--odd-pos),0);
    }
    .imgPart{  
      @include vertical-align-mixin;
      position: relative;
